...
function main($argc, $argv)
{
	...
	while (($question = $solver->backwardChain($domain, $state)) instanceof AskedQuestion)
	{
		$answer = cli_ask($question->question, $question->skippable);

		if ($answer instanceof Option)
			$state->apply($answer->consequences,
				Yes::because("User answered '{$answer->description}' to '{$question->question->description}'"));
	}
 ...
 * @return Option
 */
function cli_ask($question, $skippable)
{
